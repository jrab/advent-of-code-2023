...
interface IGames {
	[x: string]: IGameResult[];
}

const getGameResult = (gameResult: string): IGameResult => {
...
const loadGames = (gameList: string): IGames => {
...
const validateRound = (round: IGameResult, availableCubes: IGameResult): boolean => {
	// console.log(round, availableCubes);
	return (
		round.red <= availableCubes.red &&
		round.green <= availableCubes.green &&
		round.blue <= availableCubes.blue
	);
}

const getMaxColors = (result: IGameResult[]) => {
...
const getMaxColorsPerGame = (games: IGames): IGames => {
...
const validateGames = (games: IGames, availableCubes: IGameResult): IGames => {
	const gameIds = Object.keys(games);
	return gameIds.reduce((state, gameId) => {
		const gameResults = games[gameId];
		// console.log('gameResults',gameResults)
...
const sumIds = (games: IGames): number => {
	const gameIds = Object.keys(games);
	return gameIds.reduce((state, gameId) => {
		console.log(gameId);
...
function main () {
	...
	const availableCubes: IGameResult = {
		red: 12,
		green: 13,
		blue: 14
	}
